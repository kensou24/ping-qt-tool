...
const QEvent::Type LogEvent = (QEvent::Type)QEvent::registerEventType(QEvent::User + 1);

const QEvent::Type LogFailEvent = (QEvent::Type)QEvent::registerEventType(QEvent::User + 2);

class PinglogEvent : public QEvent {
public:
    PinglogEvent(const QString& log)
        : QEvent(LogEvent)
        , mlog(log)
    {
    }
...
public:
    PingFailEvent(const QString& log)
        : QEvent(LogFailEvent)
        , mlog(log)
    {
    }
...
private:
    ...
    void handle_timeout()
    {
        if (num_replies_ == 0) {
            ...
            QString info = QString("%1  Request timed out of %2")
    ...
    void handle_receive(std::size_t length)
    {
        ...
        if (is && icmp_hdr.type() == icmp_header::echo_reply && icmp_hdr.identifier() == get_identifier() && icmp_hdr.sequence_number() == sequence_number_) {
            ...
            QString log = QString("%1  bytes from %2: icmp_seq=%3, ttl=%4, time=%5 at %6")
                              .arg(
...
#else
        return static_cast<unsigned short>(::GetCurrentThreadId());
#endif
    }
